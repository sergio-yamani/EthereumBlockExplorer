                ...
                getTransactionInfos()
                    ...
                    if($scope.blockNumber!==undefined){
                        ...
                        if(info!==undefined){
                            ...
                            $scope.localdatetime = new Date(info.timestamp*1000).toLocaleString();
                       }
                    }
                    ...
                    $scope.resultReceipt = result;
 
                    $scope.contractAddress = result.contractAddress;
                    $scope.cumulativeGasUsed = result.cumulativeGasUsed;
                    if (result.logs.lenght > 0){
                        $scope.logs = JSON.stringify(result.logs);
                    }
                    else {
                        $scope.logs = 'N/A';
                    }
